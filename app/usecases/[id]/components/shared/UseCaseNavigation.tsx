...
export function UseCaseNavigation({ useCaseId, companyId, isDraft = false }: UseCaseNavigationProps) {
  ...
  const collaborationTab = useCaseNavigation.find(item => item.key === 'collaboration')
  ...
  return (
    <div className="flex justify-between">
      ...
      {collaborationTab && (
        <Link
          href={collaborationTab.href(useCaseId)}
          className={`${
            isCurrentPath(collaborationTab.key)
              ? 'border-[#0080A3] text-[#0080A3]'
              : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
          } whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm inline-flex items-center space-x-2 transition-colors`}
        >
          {getIcon(collaborationTab.key)}
          <span>{collaborationTab.label}</span>
        </Link>
      )}
    </div>
  )
