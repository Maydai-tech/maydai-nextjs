...
describe('expandE6TransparencyPackToLegacyOptionCodes', () => {
  it('mappe l’ancienne option unique vers Q1.B et Q2.B (Oui radio)', () => {
...
describe('collectE6DeclaredOptionCodes — pack transparence parcours court', () => {
  it('agrège INTERACTION + CONTENT en codes legacy pour checklist_gov_usecase', () => {
    ...
    expect(codes).toEqual(expect.arrayContaining(['E6.N10.Q1.B', 'E6.N10.Q2.B']))
    expect(codes).toHaveLength(2)
  })

  it('agrège l’ancien code unique en Q1.B + Q2.B', () => {
    const codes = collectE6DeclaredOptionCodes({
  ...
  it('fusionne codes synthétiques et réponses E6.N10.Q* déjà présentes', () => {
    ...
    expect(codes).toEqual(expect.arrayContaining(['E6.N10.Q1.B', 'E6.N10.Q2.A']))
    expect(codes).toHaveLength(2)
  })
})
