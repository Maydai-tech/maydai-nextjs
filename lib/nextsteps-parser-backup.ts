...
export function extractNextStepsFromMarkdown(reportText: string): Partial<UseCaseNextStepsInput> {
  ...
  if (prioritiesMatch) {
    ...
    if (priorityMatches && priorityMatches.length >= 3) {
      result.priorite_1 = priorityMatches[0].replace(/\*\*/g, '').replace(/^\d+\.\s*/, '').replace(/\.$/, '').trim()
      result.priorite_2 = priorityMatches[1].replace(/\*\*/g, '').replace(/^\d+\.\s*/, '').replace(/\.$/, '').trim()
      result.priorite_3 = priorityMatches[2].replace(/\*\*/g, '').replace(/^\d+\.\s*/, '').replace(/\.$/, '').trim()
    }
  }
  ...
  if (quickWinsMatch) {
    ...
    if (quickWinMatches && quickWinMatches.length >= 3) {
      result.quick_win_1 = quickWinMatches[0].replace(/\*\*/g, '').replace(/^\d+\.\s*/, '').replace(/\.$/, '').trim()
      result.quick_win_2 = quickWinMatches[1].replace(/\*\*/g, '').replace(/^\d+\.\s*/, '').replace(/\.$/, '').trim()
      result.quick_win_3 = quickWinMatches[2].replace(/\*\*/g, '').replace(/^\d+\.\s*/, '').replace(/\.$/, '').trim()
    }
  }
  ...
  if (actionsMatch) {
    ...
    // Chercher les patterns avec ou sans numérotation : **1. texte :** ou **texte :**
    const actionMatches = actionsSection.match(/\*\*(\d+\.\s*)?([^*]+)\*\*:/g)
    
    if (actionMatches && actionMatches.length >= 3) {
      result.action_1 = actionMatches[0].replace(/\*\*/g, '').replace(/^\d+\.\s*/, '').replace(/:$/, '').trim()
      result.action_2 = actionMatches[1].replace(/\*\*/g, '').replace(/^\d+\.\s*/, '').replace(/:$/, '').trim()
      result.action_3 = actionMatches[2].replace(/\*\*/g, '').replace(/^\d+\.\s*/, '').replace(/:$/, '').trim()
    }
  }
