...
function statsOrNull(xs: Array<number | null | undefined>): {
  ...
  if (values.length === 0) {
    return { mean: null, median: null, std: null, min: null, max: null, count: 0 }
  }
  return {
    mean: mean(values),
...
async function getCurrentMethodologyVersionId(supabase: Supabase): Promise<string> {
  // "courante" : on prend la plus récente capturée.
  ...
  // Fallback robuste: on crée une snapshot datée "aujourd’hui" (comme `eco-sync`).
    ...
    for (const model of models) {
      ...
      for (let run = 1; run <= runsPerModel; run++) {
        ...
        try {
          ...
          errors.push({
            modelId: model.id,
            error: `estimation run#${run}: ${e instanceof Error ? e.message : String(e)}`,
          })
          continue
        }
      ...
      // Build per KPI/split stats into `eco_evaluations_aggregated` columns (migration 019 shape)
      for (const kpi of KPIS) {
        for (const split of SPLITS) {
          const key = `${kpi}_${split}` as const
