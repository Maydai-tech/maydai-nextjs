...
function HeaderCtaLink({
  ...
  if (cta.variant === 'primary') {
    const baseClass =
      layout === 'desktop'
        ? 'px-5 py-2 rounded-lg font-semibold shadow transition text-white bg-[#ffab5a] hover:bg-[#e6995a]'
        : 'block w-full px-5 py-3 rounded-lg font-semibold shadow transition text-center text-white bg-[#ffab5a] hover:bg-[#e6995a]'
...
interface HeaderClientProps {
  headerCtas: readonly MarketingHeaderCta[]
}

export default function HeaderClient({ headerCtas }: HeaderClientProps) {
    ...
    <header className="w-full bg-white/80 backdrop-blur border-b border-gray-100 sticky top-0 z-30">
      <nav className="max-w-7xl mx-auto flex items-center justify-between px-4 py-3 md:py-4">
            ...
            if (isMarketingNavDropdown(item)) {
              ...
              return (
                <li key={item.label} className="relative">
                  <button
                    type="button"
                    onClick={() => setOpenDropdownLabel(isOpen ? null : item.label)}
        ...
        <div className="hidden md:flex gap-2 items-center">
          {headerCtas.map((cta) => (
            <HeaderCtaLink key={cta.label} cta={cta} layout="desktop" />
          ))}
        </div>
