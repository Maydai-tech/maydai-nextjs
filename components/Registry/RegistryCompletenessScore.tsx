...
interface RegistryCompletenessScoreProps {
...
function CompletenessScoreSkeleton({ label }: { label: string }) {
  return (
    <aside
...
function isScoreUnavailable(score: number | null | undefined): score is null | undefined {
  return score === null || score === undefined
}

export default function RegistryCompletenessScore({
  score,
  isLoading = false,
  onHighlightRequest,
}: RegistryCompletenessScoreProps) {
  if (isLoading || isScoreUnavailable(score)) {
    return (
      <CompletenessScoreSkeleton label="Chargement du score de complétude du registre" />
    )
  }
      ...
      <div className="flex flex-col items-center" aria-live="polite">
        ...
        <div className="relative w-20 h-20">
          <svg
            ...
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`Complétude du registre : ${clampedScore} pour cent`}
          >
            <circle
              cx={CENTER}
              cy={CENTER}
        ...
        {clampedScore === 100 ? (
          <div className="mt-4 flex items-center justify-center gap-1.5 text-emerald-600 animate-in fade-in zoom-in duration-500">
            ...
            <span className="text-xs font-medium">Registre optimal</span>
          </div>
        ) : (
          <button
            type="button"
