...
import { Users, UserX, Crown, Globe, Building2 } from 'lucide-react';
...
interface RegistryCollaboratorListProps {
...
export default function RegistryCollaboratorList({
  collaborators,
  loading = false,
  onRemove,
  emptyMessage = 'Aucun collaborateur pour ce registre'
}: RegistryCollaboratorListProps) {
  if (loading) {
    return (
  ...
  const getScopeBadge = (scope?: 'account' | 'registry') => {
    ...
    if (scope === 'account') {
      return (
        <span
          ...
          title="Accès à tous les registres (invité depuis Paramètres)"
        >
    ...
    return (
      <span
        ...
        title="Accès uniquement à ce registre"
      >
  ...
  return (
    <div className="space-y-2">
      {collaborators.map((collaborator) => {
        const canRemove = collaborator.role !== 'owner' && collaborator.scope === 'registry';
            ...
            <div className="flex items-center space-x-3 flex-1">
              ...
              {onRemove && canRemove ? (
                <button
                  ...
                  title="Retirer l'accès à ce registre"
                >
                  <UserX className="w-4 h-4" />
                </button>
              ) : (
                collaborator.role !== 'owner' && collaborator.scope === 'account' && (
                  <div className="text-xs text-gray-400 italic px-2" title="Ne peut être supprimé que depuis la page Paramètres">
                    Géré globalement
                  </div>
                )
              )}
