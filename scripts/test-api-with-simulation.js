...
/**
 * Script pour tester l'API avec simulation OpenAI
 * Usage: node scripts/test-api-with-simulation.js [usecase_id]
 */
...
async function testAPIWithSimulation(usecaseId) {
  console.log('🔍 Test de l\'API avec simulation OpenAI')
  ...
  try {
    ...
    console.log('\n🎉 Test réussi!')
    console.log('\n📝 Pour utiliser avec une vraie clé OpenAI:')
    console.log('1. Obtenez votre clé sur https://platform.openai.com/account/api-keys')
    ...
    console.log('4. Testez l\'API: curl -X POST http://localhost:3002/api/generate-report')
...
if (!usecaseId) {
  console.log('Usage: node scripts/test-api-with-simulation.js [usecase_id]')
  console.log('Exemple: node scripts/test-api-with-simulation.js dbe93d01-1b42-442a-80ab-79f71fdcd1bf')
  process.exit(1)
}

testAPIWithSimulation(usecaseId)
